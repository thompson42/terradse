...
echo
ansible-playbook dse_install.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Start DSE cluster in insecure mode... <<<<"
echo
ansible-playbook dse_cluster_start.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Configure DSE and SPARK Keyspaces - Strategy / Replication / Repair <<<<"
echo
ansible-playbook dse_keyspace_replication_configure.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Stop DSE cluster... <<<<"
echo
ansible-playbook dse_cluster_stop.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Install DSE security dependencies <<<<"
echo
ansible-playbook dse_security_install_dependencies.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Activate DSE cluster Unified Authentication <<<<"
echo
ansible-playbook dse_authentication.yml --private-key=~/.ssh/id_rsa_aws
echo

echo ">>>> Configure DSE Role Management <<<<"
echo
ansible-playbook dse_role_management.yml --private-key=~/.ssh/id_rsa_aws
echo
...
echo
ansible-playbook dse_authorisation_roles.yml --private-key=~/.ssh/id_rsa_aws
echo
...
echo
ansible-playbook jmx_authentication.yml --private-key=~/.ssh/id_rsa_aws
echo
...
echo
ansible-playbook opsc_install.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Install OPSC security dependencies <<<<"
echo
ansible-playbook opsc_security_install_dependencies.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Setup Opscenter storage cluster (OPSC DSECore) Authentication <<<<"
echo
ansible-playbook opsc_dsecore_authentication.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Start the OPSC DSECore cluster... <<<<"
echo
ansible-playbook opsc_cluster_start.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Setup OPSC DSECore cluster Authorisation/Roles <<<<"
echo
ansible-playbook opsc_dsecore_authorisation_roles.yml --private-key=~/.ssh/id_rsa_aws
echo
...
echo
ansible-playbook opsc_authorisation_roles.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Inject Opscenter cluster configuration via API call -> cluster_name.conf <<<<"
echo
ansible-playbook opsc_cluster_configure.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Inject into Opscenter services activation and other best practise configurations via API call <<<<"
echo
#ansible-playbook opsc_services_configure.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Inject into Opscenter backup schedule via API call <<<<"
echo " Disabled, must run after schema creation... "
#ansible-playbook opsc_backups_configure.yml --private-key=~/.ssh/id_rsa_aws
echo
...
echo
ansible-playbook spark_datacenter_stop.yml --private-key=~/.ssh/id_rsa_aws
echo
...
echo
ansible-playbook spark_authentication.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Configure DSEFS, AlwaysOnSQL, Worker cleanup and Logging in the Spark DC <<<<"
echo
ansible-playbook spark_configure.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Start analyics datacenter <<<<"
echo
ansible-playbook spark_datacenter_start.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Configure Search Authorisation and Roles - DISABLED <<<<"
echo
#ansible-playbook search_authorisation_roles.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Configure Spark Authorisation and Roles - DISABLED <<<<"
echo
#ansible-playbook spark_authorisation_roles.yml --private-key=~/.ssh/id_rsa_aws
echo

echo
echo ">>>> Configure Graph Authorisation and Roles - DISABLED <<<<"
echo
#ansible-playbook graph_authorisation_roles.yml --private-key=~/.ssh/id_rsa_aws
echo
