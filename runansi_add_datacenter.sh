...
# SHUTDOWN NODES IN THE NEW DATACENTER

echo
echo ">>>> Activate Unified Authentication on new Datacenter nodes <<<<"
echo
#ansible-playbook -i hosts add_datacenter_authentication.yml --private-key=~/.ssh/id_rsa_aws -- NOT DONE YET
echo

echo
echo ">>>> Activate JMX Unified Authentication on new Datacenter nodes <<<<"
echo
#ansible-playbook -i hosts add_datacenter_jmx_authentication.yml --private-key=~/.ssh/id_rsa_aws -- NOT DONE YET
echo
